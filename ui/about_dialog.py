"""Bookmark Tool（书签工具）的"关于"对话框。

使用自定义 QDialog 而非 QMessageBox.about，以便：
- 配置目录展示通用形式（%LOCALAPPDATA%\\BookmarkTool 等），不暴露用户名；
- 提供可点击的 GitHub 仓库超链接；
- 更宽松的排版控制。
"""
...
def _generic_config_dir() -> str:
    ...
    if sys.platform.startswith("win"):
        return r"%LOCALAPPDATA%\BookmarkTool"
    if sys.platform == "darwin":
        return "~/Library/Application Support/BookmarkTool"
    return "~/.config/BookmarkTool"


def _masked_config_dir() -> str:
    """实际路径，但把用户名替换成占位符，便于截图分享而不泄露账号名。"""
    ...
    try:
        ...
        if home and path.startswith(home):
            ...
            return os.path.join(parent, "<用户名>", rest) if rest else \
                os.path.join(parent, "<用户名>")
...
class AboutDialog(QDialog):
    ...
    def __init__(self, parent: QWidget | None = None, version: str = "") -> None:
        ...
        # 标题
        title = QLabel(
            f"<h2 style='margin:0 0 4px 0;'>Bookmark Tool · 书签工具</h2>"
        )
