...
class SettingsDialog(QDialog):
    ...
    def _ai_tab(self):
        ...
        self.cb_ai_judge = QCheckBox(
            "允许用 AI 复检「存疑」链接（读已抓取的正文判定，不二次抓 URL）")
        self.cb_ai_judge.setChecked(bool(self.cfg.get("ai_judge_enabled", False)))
        f.addRow("", self.cb_ai_judge)
    ...
    def _collect(self) -> Dict:
        ...
        cfg.update({
            ...
            "ai_judge_enabled": self.cb_ai_judge.isChecked(),
...
class TaxonomyDialog(QDialog):
    ...
    def _save_current(self):
        ...
        # 保留 description（分类说明）：编辑器不直接编辑它，但保存时不能弄丢，
        # 否则 AI 分类 prompt 会失去类别边界信息。
        if prev.get("description"):
            rules["description"] = prev["description"]
        if new != old:
    ...
    def _reset_default(self):
        """一键恢复内置默认分类体系，丢弃当前自定义改动（确认后执行）。"""
        from core import classifier
        n = len(classifier.DEFAULT_TAXONOMY)
        if QMessageBox.question(
                self, "恢复默认分类",
                f"将丢弃当前的自定义分类，恢复为内置的 {n} 个默认分类，是否继续？",
                QMessageBox.Yes | QMessageBox.No, QMessageBox.No) != QMessageBox.Yes:
            return
        self.taxonomy = json.loads(
            json.dumps(classifier.DEFAULT_TAXONOMY, ensure_ascii=False))
        self.list.clear()
        for name in self.taxonomy:
            self.list.addItem(name)
        if self.list.count():
