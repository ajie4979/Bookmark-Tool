...
SUBTYPES = [
    ST_LIMITED, ST_BLOCKED, ST_SERVER, ST_TIMEOUT, ST_UNREACHABLE, ST_TLS,
    ST_SOFT404, ST_CONFLICT, ST_NOTFOUND, ST_NODNS,
    ST_MANUAL_OK, ST_MANUAL_DEAD, ST_RULE_SKIP,
]
...
STATUS_HINT = {
    V_OK: "服务器有响应（2xx/3xx/401/403/451/5xx 或 TLS 握手），站点活着、可访问",
...
class Probe:
    ...
    @property
    def ok(self) -> bool:
        return 200 <= self.status_code < 400 and not self.soft404
...
class Bookmark:
    ...
    def merge_verdict(self):
        ...
        if codes:
            if soft404:
                sub = ST_SOFT404
            elif any(c in (401, 403, 429) for c in codes):
        ...
        # ⑤ TLS/证书：服务器回了握手 = 活着 → 可访问（中）
        #    （常见于国密站点、反爬 WAF、代理 TLS 限制，浏览器通常能正常打开）
        if any(("SSL" in e or "证书" in e or "TLS" in e) for e in errors):
            self.verdict, self.subtype, self.confidence = V_OK, ST_TLS, CONF_MID
            return
