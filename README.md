# 书签医生（Bookmark Doctor）

一个 Windows 桌面端的离线书签治理工具。读取浏览器导出的书签文件，
完成 **去重 → 可达性验证 → 重新归类 → 生成导航网页 / 导回浏览器** 的完整闭环。

名字含义：核心动作是给书签"体检"（验证可达性）+ "治疗"（去重、归类、导出），
"存疑 / 确诊 / 复检"这套术语与医疗流程天然契合。
...
启动 `dist/书签医生/书签医生.exe`（目录分发），无需安装 Python 或任何依赖。
...
### 失效链接检测
先发 HEAD，遇到 403/405/501 自动降级为 GET（只取头部不下载正文）。状态细分，避免误杀：

检测结论只有三档，尽量把「活着但程序打不开」和「真的死了」分开：

| 结论 | 含义 | 算失效吗 |
|------|------|---------|
| **可访问** | 服务器有响应：2xx / 3xx，或 401/403/429/451/5xx，甚至 TLS 握手失败 | 否 |
| **存疑** | 只有超时 / 完全无响应，程序连不上 | 否（分不出来，建议换网络复检） |
| **已失效** | 404 / 410，或域名解析失败、端口/连接被拒 | 是 |
| **未检测** | 还没跑 | 不适用 |
| **跳过** | `chrome://`、`javascript:`、本地文件、命中跳过规则 | 不适用 |

> 子类型会记下具体原因（如「访问受限 403」「地区限制 451」「服务端错误 5xx」「域名不存在」「环境矛盾」等），在表格的「结论」列悬停 / 筛选时可以看到，方便判断要不要人工复核。

### 关于「访问受限 / 地区限制 / 服务端错误」

站点返回 403 / 451 / 5xx，往往只是**拒绝了程序访问**或**需要特定网络环境**
（Cloudflare 挑战、缺 Cookie、风控、地区/法律限制、对方服务器临时故障），
用浏览器打开通常是正常的。所以这些都归为「可访问」，不会出现在「只看确认失效」里。
...
- **本地规则归类**：内置 16 个分类的规则库（域名特征 + 关键词 + 原文件夹弱信号），
  打分取最高。867 条约 0.1 秒完成，实测未分类率约 11%。

内置分类：AI 与机器学习、3D 与图形创作、设计与素材、开发与技术、前端与网页、
数据可视化、智慧城市与GIS、摄影与后期、学习与文档、工具与效率、资讯与社区、
影音娱乐、电商与生活、政府与机构、玩机与刷机、其他未分类。
...
bookmark-doctor/
├── app.py               入口
├── config.py            配置读写（存 %LOCALAPPDATA%\BookmarkDoctor\）
├── core/
...
# 目录分发打包
python -m PyInstaller --noconfirm --windowed --name BookmarkDoctor --onedir \
...
# 打包后把 BookmarkDoctor.exe 改名为 书签医生.exe
# 外层目录也可改名为「书签医生」方便用户辨认
```

## 说明

- 配置与分类体系保存在 `%LOCALAPPDATA%\BookmarkDoctor\`（macOS：`~/Library/Application Support/BookmarkDoctor`，Linux：`~/.config/BookmarkDoctor`），删掉即恢复默认
